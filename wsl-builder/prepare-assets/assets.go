...
func generateMetaFilesForRelease(r wslReleaseInfo, wslPath, rootPath string) (err error) {
	...
	err = filepath.WalkDir(rootPath, func(path string, d fs.DirEntry, err error) error {
		...
		if filepath.Ext(path) == ".png" && strings.HasPrefix(relPath, "meta/store") {
		...
		if filepath.Base(path) == "ProductDescription.xml" {
			...
			screenshotsInStore, err := filterAndSortImagesInDir(filepath.Dir(path), nil)
			if err != nil {
				return err
			}

			// And now collect screenshots already copied in the dest store directory
			screenshots, err := filterAndSortImagesInDir(filepath.Dir(path), screenshotsInStore)
			if err != nil {
				return err
			}
			// Append base sorted screenshots
			screenshots = append(screenshots, screenshotsInStore...)
