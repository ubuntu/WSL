...
type WslReleaseInfo struct {
	...
	PointRelease        int      // The point/minor release number. For instance, for 20.04.5, the point release is 5.
	...
	for _, release := range releases {
		...
		if isDevelopmentRelease {
			wsl, err := newWslReleaseInfo("Ubuntu-Preview", "UbuntuPreview", "Ubuntu (Preview)", buildVersion, "ubuntupreview",
				release[0], release[0], minor, "Preview", codeName, []string{"Ubuntu (Preview)"}, true)
			if err != nil {
				return nil, err
			}
		...
		// we don’t want to display in FullName, IconVersion or ReleaseVersion the .0 suffix. BuildVersion stays as it.
...
func newWslReleaseInfo(wslID, appID, fullName, buildVersion, launcherName, shortVersion, iconVersion string, minor int, releaseVersion, codeName string, reservedNames []string, shouldBuild bool) (WslReleaseInfo, error) {
	w := WslReleaseInfo{
		WslID:          wslID,
		AppID:          appID,
		FullName:       fullName,
...
// The base image name is in the format: ubuntu-<version>-wsl-<arch>.wsl
// Supported releases are 20.04 and later available at per-architecture URLS:
// ARM64 releases are available at https://cdimage.ubuntu.com/releases/<codename>/release/ubuntu-<version>-wsl-arm64.wsl
// AMD64 releases are available at https://releases.ubuntu.com/<version>/ubuntu-<version>-wsl-amd64.wsl
// No other architectures are supported.
func (w *WslReleaseInfo) RootfsURL(arch string) string {
	// We want daily images instead of those already published.
	if w.ShouldBuild {
		if w.PointRelease > 0 { // Already released
			return fmt.Sprintf("https://cdimages.ubuntu.com/ubuntu-wsl/%s/daily-live/current/%s-wsl-%s.wsl", w.CodeName, w.CodeName, arch)
		}
		return fmt.Sprintf("https://cdimages.ubuntu.com/ubuntu-wsl/daily-live/current/ubuntu-wsl-%s-%s.wsl", w.CodeName, arch)
	}
	// Release version without the potential LTS suffix:
	version := strings.TrimSuffix(w.ReleaseVersion, " LTS")
	switch arch {
	case "amd64", "x86_64":
		return fmt.Sprintf("https://releases.ubuntu.com/%s/ubuntu-%s-wsl-amd64.wsl", w.CodeName, version)
	case "arm64", "aarch64":
		return fmt.Sprintf("https://cdimage.ubuntu.com/releases/%s/release/ubuntu-%s-wsl-arm64.wsl", w.CodeName, version)
	default:
		return ""
	}
}
