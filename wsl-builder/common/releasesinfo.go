...
// newWslReleaseInfo creates a new WSL release info from parameters and compute the
func newWslReleaseInfo(wslID, fullName, buildVersion, launcherName, shortVersion, releaseVersion, codeName string, reservedNames []string, shouldBuild bool) (WslReleaseInfo, error) {
	...
	// Compute terminalProfileGUID for Windows Terminal as in
	...
	ut16LeEncodedMessage, err := utf16le.NewEncoder().String(wslID)
	if err != nil {
		return WslReleaseInfo{}, err
	}
	...
	wsl := WslReleaseInfo{
		...
		TerminalProfileGUID: fmt.Sprintf("{%s}", terminalProfileGUID.String()),
